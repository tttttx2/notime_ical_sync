 ...
 * changes to allow multiple events by tttttx2
 * 
 ...
 *   $ics = new ICS(array($props));
 *   $ics_file_contents = $ics->to_string();
 *
 * Setting properties after instantiation
 *   $ics = new ICS();
 *   $ics->set('summary', 'My awesome event', false, eventID);
 *
...
class ICS {
  ...
  public function set($key, $val = false, $event=0) {
    //print_r($key);
    if (is_array($key)) {
      foreach ($key as $k => $v) {
        $this->set($k, $v, $event);
      ...
      if (in_array($key, $this->available_properties)) {
        $this->properties[$event][$key] = $this->sanitize_val($val, $key);
      }
    }
  }
